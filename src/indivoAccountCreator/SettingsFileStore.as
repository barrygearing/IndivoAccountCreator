...
package indivoAccountCreator
{
	...
	public class SettingsFileStore
	{
		...
		{
			if (applicationSettingsEmbeddedFile)
		...
		private function readSettingsFromFile(file:File):Boolean
		{
			...
			var settingsXmlString:String = fileStream.readUTFBytes(fileStream.bytesAvailable);
			var settingsXml:XML;
			try
			{
				settingsXml = XML(settingsXmlString);
			}
			catch (e:TypeError)
			{
				trace("Failed to parse settings XML: ", file.nativePath, "\n" + settingsXmlString);
				return false;
			}
			finally
			{
			...
			return decodeXML("Settings file \"" + file.nativePath + "\"", settingsXml);
		}
		...
		private function readSettingsFromEmbeddedFile(embeddedFile:Class):Boolean
		{
			...
			var settingsXml:XML = XML(byteArray.readUTFBytes(byteArray.length));
			return decodeXML("Embedded (application level) settings.xml file", settingsXml);
		}
...
//			xmlEncoder.setAttribute(xmlList[0], "xmlns", schema.namespaces[""].uri);
		...
		{
			var file:File = userSettingsFile;
			var fileStream:FileStream = new FileStream();
			...
			{
				trace("File exists but could not be opened for writing: ", file.nativePath);
				return false;
			}

			fileStream.writeUTFBytes(encodeToXML());
			fileStream.close();
			return true;
		}
	}
